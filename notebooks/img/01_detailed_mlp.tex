...
\providecommand{\hidlText}[1]{
 {$H_{#1} = \sum_{j} W^{I}_{j#1} \cdot x_{j} + b^{I}_{#1}$}%
}
\providecommand{\hidrText}[1]{
 {$h_{#1}=\sigma_{relu}(H_{#1})$}%
}
\providecommand{\outlText}[1]{
 {$y'_{#1} = \sum_{j} W^{II}_{j#1} \cdot h_{j} + b^{II}_{#1}$}%
}
...
\begin{tikzpicture} [
  ...
  \node[hiddenNeuron, right=6em of relG 01] (outl 0) {\outlText{0}}; 
  \node[hiddenNeuron, right=6em of relG 12] (outl 1) {\outlText{1}};

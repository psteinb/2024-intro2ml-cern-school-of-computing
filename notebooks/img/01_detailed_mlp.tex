...
\providecommand{\hidlText}[1]{
 {$H_{#1} = \sum_{j} \mathbf{\omega}^{I}_{j#1} \cdot x_{j} + \beta^{I}_{#1}$}%
}
\providecommand{\hidrText}[1]{
 {$h_{#1}=\sigma_{relu}(H_{#1})$}%
}
\providecommand{\outlText}[1]{
 {$y'_{#1} = \sum_{j} \mathbf{\omega}^{II}_{j#1} \cdot h_{j} + \beta^{II}_{#1}$}%
}
...
\begin{tikzpicture} [
  ...
  \foreach \a in {0,...,2} {
   \foreach \s in {0,...,1} {
    \draw (rel \a) -- (outl \s) node[synapseNodeMy] {\synapseHOText{\a}{\s}
    };
   }}
